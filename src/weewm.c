...
int NumLockMask;                        /* num lock mask                */
int CapsLockMask;                       /* caps lock mask               */
int ScrollLockMask;                     /* scroll lock mask             */
...
{
    int i, config_file_length, keys_file_length, memo_file_length;
    ...
    for (i = 1; i < argc; i++)
    {
            ...
            || (strcmp (argv[i], "--config") == 0) )
        {
            if (i == argc - 1)
                fprintf (stderr,
                         "WeeWM: warning: no config file specified "
                         "(parameter '%s'), ignored\n",
                         argv[i]);
            else
            {
            ...
            || (strcmp (argv[i], "--keys") == 0) )
        {
            if (i == argc - 1)
                fprintf (stderr,
                         "WeeWM: warning: no keys file specified "
                         "(parameter '%s'), ignored\n",
                         argv[i]);
            else
            {
            ...
            || (strcmp(argv[i], "--font") == 0) )
        {
            if (i == argc - 1)
                fprintf (stderr,
                         "WeeWM: warning: no font specified "
                         "(parameter '%s'), ignored\n",
                         argv[i]);
            else
            {
            ...
            || (strcmp (argv[i], "--licence") == 0) )
        {
            ...
            || (strcmp (argv[i], "--list-commands") == 0) )
        {
            wee_print_commands ();
            exit (EXIT_SUCCESS);
        }
        else if ( (strcmp (argv[i], "-o") == 0)
            || (strcmp (argv[i], "--list-options") == 0) )
        {
            wee_print_options ();
            exit (EXIT_SUCCESS);
        }
            ...
            || (strcmp (argv[i], "--display") == 0) )
        {
            if (i == argc - 1)
                fprintf (stderr,
                         "WeeWM: warning: no display specified "
                         "(parameter '%s'), ignored\n",
                         argv[i]);
            else
            {
        ...
        {
            fprintf (stderr,
                     "WeeWM: warning: unknown parameter '%s', ignored\n",
                     argv[i]);
        }
    }
    ...
    if (config_file == NULL)
    {
        config_file_length = strlen (getenv ("HOME")) + 64;
        config_file = (char *)malloc(config_file_length * sizeof (char));
        snprintf (config_file, config_file_length,
                  "%s/.weewm/config.rc", getenv("HOME"));
    ...
    if (keys_file == NULL)
    {
        keys_file_length = strlen (getenv ("HOME")) + 64;
        keys_file = (char *)malloc(keys_file_length * sizeof (char));
        snprintf(keys_file, keys_file_length,
                 "%s/.weewm/keys.rc", getenv("HOME"));
        default_keys_name = 1;
    }
    else
        default_keys_name = 0;
    
    memo_file_length = strlen (getenv ("HOME")) + 64;
    memo_file = (char *)malloc(memo_file_length * sizeof (char));
    snprintf(memo_file, memo_file_length,
             "%s/.weewm/memo_app", getenv("HOME"));
}
...
{
    char *weewm_home_dir;
    int weewm_home_dir_length;
    int return_code;
    
    weewm_home_dir_length = strlen (getenv ("HOME")) + 64;
    weewm_home_dir = (char *)malloc(weewm_home_dir_length * sizeof (char));
    snprintf(weewm_home_dir, weewm_home_dir_length,
             "%s/.weewm", getenv("HOME"));
    ...
    if (return_code < 0)
    {
        ...
        {
            fprintf (stderr,
                     "WeeWM: can't create directory '%s'.\n", weewm_home_dir);
            free (weewm_home_dir);
            exit (1);
        }
    }
...
int handle_xerror (Display *display, XErrorEvent *event)
{
    ...
    client = client_search (event->resourceid);
    ...
    for (i = 0; i < num_heads; i++)
    {
        ...
        for (j = 0; j < nchildren; j++)
        {
            ...
            if (!attr.override_redirect && attr.map_state == IsViewable)
                client_new (children[j], i);
        }
        XFree (children);
        XChangeWindowAttributes (display, RootWindow (display, i),
                                 CWEventMask, &sattr);
        ...
        for (ptr_keymap = keymap; ptr_keymap != NULL; ptr_keymap = ptr_keymap->next)
        {
            ...
            {
                XGrabKey (display,
                          XKeysymToKeycode (display, ptr_keymap->keysym),
                          ignore_modifiers[j] | ptr_keymap->modifier_mask,
                          RootWindow (display, i), True,
