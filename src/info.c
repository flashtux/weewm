...
void info_add (char *message, int screen)
{
    ...
    if (!info_exists)
    {
        ...
        info_window = XCreateWindow (display, RootWindow(display, screen), 0,
                                     DisplayHeight(display, screen) - config_info_height,
    ...
    /*if (info_message)
    {
        XDrawString (display, RootWindow(display, screen),
                     invert_gc[screen],
                     10, get_y_max(screen)-20, info_message,
                     strlen (info_message));
        free (info_message);
    }
    info_message = strdup (message);
    XDrawString (display, RootWindow(display, screen),
                 invert_gc[screen],
                 10, get_y_max(screen)-20, info_message,
                 strlen (info_message));*/
