...
{ { "next_win",           action_next_win,              "focus next window"                       },
  ...
  { "win_smart_max",      action_smart_maximize,        "smart maximize window"                   },
...
void wee_print_commands ()
{
    ...
    {
        printf("%20s: %s\n", weewm_commands[i].command_name,
               weewm_commands[i].description);
        i++;
    }
}
...
char **explode_string (char *string, char *separators, int num_items_max,
                       int *num_items)
{
    int i, n_items;
    char **array;
...
void config_set_option (char **items)
{
    ...
    {
        fprintf (stderr,
                 "WeeWM: value missing for option '%s', line discarded.\n",
                 items[0]);
        return;
    }
    i = 0;
            ...
            switch (weewm_options[i].option_type)
            {
                case OPTION_TYPE_INT:
                    *weewm_options[i].ptr_int = atoi(items[1]);
                    ...
                    {
                        fprintf (stderr,
                                 "WeeWM: option '%s' out of range. "
                                 "Setting it to %d (default value).\n",
                ...
                case OPTION_TYPE_STRING:
                    ...
                    *weewm_options[i].ptr_string =
                        (char *)(malloc ((strlen (items[1])+1)*sizeof (char)));
...
t_keymap *create_new_key_binding (t_keymap *km, int num_items, char **items)
{
    ...
    if (strncasecmp (items[2], "run", 3) == 0)
    {
        ...
        key->application_name = strdup (items[3]);
        key->initial_desktop = atoi(items[4]);
        key->smart_position = strdup (items[5]);
        key->only_one = (strcasecmp (items[2], "run1") == 0) ? 1 : 0;
        key->command_file = strdup (items[6]);
        key->command_args = (char **)malloc ((num_items - 7 + 1 + 1) * sizeof(char *));
        key->command_args[0] = strdup (items[6]);
        
        i = 7;
        while (items[i] != NULL)
        {
            key->command_args[i-7+1] = strdup (items[i]);
            i++;
        }
    ...
    else
    {
        ...
        if (strncasecmp (items[2], "desktop_", 8) == 0)
        {
            ...
            if ( (key->key_switch_desktop < 0)
                || (key->key_switch_desktop > MAX_DESKTOPS-1) )
            {
                fprintf(stderr,
                        "WeeWM: invalid desktop number for desktop_xxx command"
...
void print_keymap (t_keymap *km)
{
    ...
    while (km)
    {
        printf ("WeeWM: %s(%d),%s(%d),dsk:%d,smart:%s,cmd:%s",
...
{
    FILE *file;
    char line[LINE_MAX_LENGTH], *ptr_line;
    char *separators = ":= \t";
    char **array_items;
    int i, num_items;
                ...
                if (array_items != NULL)
                {
                    ...
                    if (config_type == CONFIG_KEYS)
                    {
                        if (strcasecmp (array_items[0], "mouse_mask") == 0)
                            ...
                            keymap = create_new_key_binding (keymap, num_items,
                                                             array_items);
                    }
                }
                i = 0;
...
void wee_create_default_config (char *filename, int config_type)
{
    ...
    {
        case CONFIG_OPTIONS:
            fprintf (file, "# WeeWM configuration\n");
            fprintf (file, "# See README for syntax.\n\n");
            fprintf (file, "# ===== WeeWM configuration =====\n\n");
            i = 0;
            while (weewm_options[i].option_name)
            {
            ...
            break;
        case CONFIG_KEYS:
            fprintf (file, "# WeeWM key bindings\n");
            fprintf (file, "# See README for syntax.\n\n");
            fprintf (file, "# ===== WeeWM key bindings =====\n\n");
            i = 0;
            while (weewm_default_keys[i].key_name)
            {
                if (weewm_default_keys[i].application_name)
                    fprintf (file, "%-25s %-10s %-5s %s %d %s %s\n",
